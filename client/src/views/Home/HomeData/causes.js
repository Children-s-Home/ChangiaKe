import pic1 from '../assets/pexels-pixabay-45842 1.png';
import pic2 from '../assets/pexels-rdne-stock-project-8581019 1.png';
import pic3 from '../assets/pexels-pixabay-45842 3.png';

export const causes = [
  {
    pic: pic1,
    location: 'Kibera',
    title: 'Providing healthy food for children',
    paragraph:
      'Your donation will help us to continue our work and make a difference in the world. Every dollar counts, and we are grateful for your support.',
  },
  {
    pic: pic2,
    location: 'Ngong',
    title: "Providing shoes to the kids in Hope children's home.",
    paragraph:
      'We can come together and contribute to support our communities. Together, we can make a difference.',
  },
  {
    pic: pic3,
    location: 'Syokimau',
    title: 'Picking up trash around Syokimau estate.',
    paragraph:
      'If you are interested in volunteering, please contact us. We have a variety of volunteer opportunities available.',
  },
  {
    pic: pic3,
    location: 'Syokimau',
    title: 'Picking up trash around Syokimau estate.',
    paragraph:
      'If you are interested in volunteering, please contact us. We have a variety of volunteer opportunities available.',
  },
  {
    pic: pic3,
    location: 'Syokimau',
    title: 'Picking up trash around Syokimau estate.',
    paragraph:
      'If you are interested in volunteering, please contact us. We have a variety of volunteer opportunities available.',
  },
];
